...
const engines = [
  {
    ...
    description: "Strategic accumulation of infrastructure-grade digital assets through dollar-cost averaging and conviction-based allocation. Long-term exposure to blockchain adoption appreciation.",
  ...
  },
  {
    ...
    description: "Protocol-native yield generation through staking, lending, and liquidity provision. Generates operational revenue from held assets rather than leaving them idle.",
  ...
  },
  {
    ...
    description: "Quantitative and discretionary trading across market cycles. Captures short-to-medium term alpha while the core portfolio compounds through infrastructure adoption.",
  ...
  },
  {
    ...
    description: "Dynamic rebalancing between asset categories based on macro signals, adoption milestones, and risk regime changes. Active management of portfolio composition.",
  ...
  },
  {
    ...
    description: "Position sizing, drawdown limits, and correlation management to protect capital through market volatility. Institutional-grade risk infrastructure across all strategies.",
  ...
  },
  {
    ...
    description: "Optional future exposure to tokenization services, validator operations, and institutional blockchain services. An emerging optionality layer for the operating model.",
        ...
        <div className="flex flex-col md:flex-row items-center justify-center gap-3 md:gap-4 text-center">
          {[
            ...
            <div key={step.label} className="flex items-center gap-3 md:gap-4">
              <div className="rounded-lg border border-white/[0.08] bg-white/[0.03] px-4 py-3 min-w-[160px]">
                <div className="text-xs font-medium text-white">{step.label}</div>
                <div className="text-[10px] text-zinc-500 mt-0.5">{step.sub}</div>
              </div>
              {i < 3 && <ArrowRight className="h-4 w-4 text-zinc-600 shrink-0 hidden md:block" />}
              {i < 3 && <ArrowDown className="h-4 w-4 text-zinc-600 shrink-0 md:hidden" />}
            </div>
          ))}
        </div>
      </motion.div>
