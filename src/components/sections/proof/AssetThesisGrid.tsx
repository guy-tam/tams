...
function AssetCard({ profile, onAssetClick, t }: { profile: AssetThesisProfile; onAssetClick?: (ticker: string) => void; t: (key: string) => string }) {
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className="rounded-xl border border-amber-500/15 bg-white/[0.03] backdrop-blur-sm overflow-hidden hover:border-amber-500/25 transition-colors duration-300"
    >
      <div className="p-5">
        {/* כותרת */}
        <div className="flex items-start justify-between mb-3">
          ...
          >
            ...
            <div className="text-[9px] text-zinc-500 group-hover:text-amber-500/60 transition-colors">{t("proof.assetGrid.view")} &darr;</div>
          </button>
        </div>
