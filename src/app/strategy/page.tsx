...
export default function StrategyPage() {
  ...
  const strategySections = [
    { id: "strategy-hero", label: "אסטרטגיה" },
  ...
  return (
    <PageWrapper bgGrid>
      <PageTableOfContents sections={strategySections} />
      ...
      <section id="strategy-tam" className="py-12 max-w-7xl mx-auto">
        <SectionHeader badge={t("strategy.tam.badge")} title={t("strategy.tam.title")} subtitle={t("strategy.tam.subtitle")} />
        <TAMSAMSOMPanel />
      </section>

      <section id="strategy-pos" className="py-16 max-w-4xl mx-auto">
        <SectionHeader badge={t("strategy.pos.badge")} title={t("strategy.pos.title")} subtitle={t("strategy.pos.subtitle")} />
        <div className="space-y-4">
          {[0, 1, 2, 3].map((i) => (
            <motion.div key={i} initial={{ opacity: 0, x: -16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.08 }} className="flex gap-4 items-start rounded-xl border border-amber-500/15 bg-white/[0.04] backdrop-blur-md p-5">
      ...
      <section id="strategy-poc" className="py-12 max-w-4xl mx-auto">
      ...
      <section id="strategy-edge" className="py-16 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {advantages.map((adv, i) => {
            const Icon = adv.icon;
            return (
              <motion.div key={i} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.1 }} className="rounded-2xl border border-amber-500/15 bg-white/[0.04] backdrop-blur-md p-6">
